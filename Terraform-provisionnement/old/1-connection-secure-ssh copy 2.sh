...
sync_ssh_keys_to_user() {
    ...
    if [ -f "$USER_PRIVATE_KEY" ] || [ -f "$USER_PUBLIC_KEY" ]; then
        ...
        echo "   ⚠️  Clés existantes détectées → sauvegarde..."
    ...
    # 🔍 Vérifier s'il existe des fichiers .bak dans ~/.ssh
    BAK_FILES=$(find "$USER_SSH_DIR" -maxdepth 1 -type f -name "*.bak" 2>/dev/null || true)

    if [ -n "$BAK_FILES" ]; then
        echo "   📦 Fichiers .bak détectés → création du dossier backup/"

        mkdir -p "$BACKUP_DIR"
        chmod 700 "$BACKUP_DIR"

        echo "$BAK_FILES" | while read -r bak; do
            [ -n "$bak" ] && mv "$bak" "$BACKUP_DIR/"
            echo "   ➜ Déplacé : $(basename "$bak") → backup/"
        done
    fi
    ...
    echo "   ✔️ Clés synchronisées dans ~/.ssh/"
}
