...
# =============================================================================
# FONCTION: create_optimized_cloudinit
# Description: Crée un fichier cloud-init optimisé pour démarrage rapide
# Arguments: vm_id, output_dir
# =============================================================================
create_optimized_cloudinit() {
    local VM_ID="$1"
    local OUTPUT_DIR="${2:-/var/lib/vz/template/cloud-init-images}"
    local CI_DIR="$OUTPUT_DIR/cloud-init-custom"
    
    # Créer le répertoire
    mkdir -p "$CI_DIR"
    
    # Fichier user-data optimisé (démarrage rapide)
    cat > "$CI_DIR/user-data-$VM_ID" << 'EOF'
#cloud-config
# =============================================================================
# Cloud-init ULTRA-OPTIMISÉ pour Proxmox
# Objectif: Minimiser le temps de démarrage
# =============================================================================
...
# --- Désactiver le growpart (déjà redimensionné dans le template) ---
growpart:
  mode: off

# --- Réduire les modules exécutés (gain ~30s) ---
cloud_init_modules:
  - migrator
  - seed_random
  - resizefs
...
bootcmd:
  - echo "Demarrage rapide cloud-init" > /dev/ttyS0

# --- Pas d'attente pour le réseau (gain ~5-15s) ---
network:
  config: disabled

# --- QEMU Guest Agent seulement (essentiel) ---
packages:
  - qemu-guest-agent
...
EOF
    ...
    # Fichier meta-data minimal
    cat > "$CI_DIR/meta-data-$VM_ID" << EOF
instance-id: template-$VM_ID
local-hostname: template-$VM_ID
EOF

    # Créer l'ISO cloud-init
    local ISO_PATH="$OUTPUT_DIR/cloud-init-$VM_ID.iso"
    
    # Utiliser cloud-localds si disponible, sinon fallback sur genisoimage/mkisofs
    if command -v cloud-localds &> /dev/null; then
        cloud-localds "$ISO_PATH" "$CI_DIR/user-data-$VM_ID" "$CI_DIR/meta-data-$VM_ID" 2>/dev/null || true
    elif command -v genisoimage &> /dev/null; then
        genisoimage -output "$ISO_PATH" -volid cidata -joliet -rock "$CI_DIR/user-data-$VM_ID" "$CI_DIR/meta-data-$VM_ID" 2>/dev/null || true
    elif command -v mkisofs &> /dev/null; then
        mkisofs -output "$ISO_PATH" -volid cidata -joliet -rock "$CI_DIR/user-data-$VM_ID" "$CI_DIR/meta-data-$VM_ID" 2>/dev/null || true
    fi
    
    # Retourner le chemin de l'ISO si créée
    if [ -f "$ISO_PATH" ]; then
        echo "$ISO_PATH"
        return 0
    else
...
create_template() {
    ...
    # =====================
    # CLOUD-INIT CONFIG (OPTIMISÉ)
    # =====================
    echo "☁️ Configuration cloud-init optimisée..."
    
    # Créer l'ISO cloud-init optimisé
    local CI_ISO_PATH=$(create_optimized_cloudinit "$VM_ID" "$IMAGE_SEARCH_DIR")
    
    if [ -n "$CI_ISO_PATH" ] && [ -f "$CI_ISO_PATH" ]; then
        echo "   ✅ ISO cloud-init optimisé: $CI_ISO_PATH"
        # Attacher l'ISO cloud-init personnalisé en ide2
        qm set $VM_ID \
            --ide2 "$STORAGE:cloudinit,media=cdrom" \
            --boot order=scsi0 \
            --vga std
        
        # Copier l'ISO dans le storage Proxmox pour qu'il soit accessible
        local ISO_FILENAME="cloud-init-$VM_ID.iso"
        local ISO_STORAGE_PATH="$STORAGE:iso/$ISO_FILENAME"
        
        # Vérifier si le répertoire iso existe sur le storage
        if pvesm path "$STORAGE" &>/dev/null; then
            local STORAGE_PATH=$(pvesm path "$STORAGE")
            if [ -d "$STORAGE_PATH/template/iso" ]; then
                cp "$CI_ISO_PATH" "$STORAGE_PATH/template/iso/$ISO_FILENAME" 2>/dev/null || true
            fi
        fi
    else
        echo "   ⚠️ ISO non créée, utilisation du cloud-init standard"
        qm set $VM_ID \
