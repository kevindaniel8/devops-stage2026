...
| Domaine | `https://argo.greencontracts.org` | Accès production (via Ingress) |
...
argocd_domain: "argo.{{ domain | default('greencontracts.org') }}"  # URL d'accès
...
```yaml
domain: "greencontracts.org"               # Utilisé par argocd_domain
```
...
5. **Ingress** pour accès domaine (`argo.greencontracts.org`)
