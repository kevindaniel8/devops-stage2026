...
OUTPUT_FILE="${2:-"$REMOTE_DIR/terraform-config.txt"}"
...
# error() { echo -e "\033[1;31m[ERROR $1]\033[0m ${2}"; } # sortie standart standout creer un bug 
...
install_ssh_key() {
    ...
    ############################################
    # 4.3 — COPIE DE SÉCURITÉ DANS $REMOTE_DIR/ssh
    ############################################
    BACKUP_USER_DIR="$REMOTE_DIR/ssh"
