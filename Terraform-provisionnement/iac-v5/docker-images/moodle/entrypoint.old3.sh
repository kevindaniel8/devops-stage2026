...
# Configurer les liens symboliques pour Moodle 5.2 (structure public)
# Moodle 5.2 a tout le code dans public/, les liens doivent pointer du parent vers public/
PUBLIC_DIR="/var/www/moodle/public"

if [ -d "${PUBLIC_DIR}" ]; then
    echo "Configuration des liens symboliques pour Moodle 5.2..."
    
    # Créer des liens symboliques du parent VERS public/ pour les includes
    for dir in lib course mod blocks admin theme pix login backup calendar completion; do
        if [ -d "${PUBLIC_DIR}/$dir" ] && [ ! -e "/var/www/moodle/$dir" ]; then
            ln -sf "${PUBLIC_DIR}/$dir" "/var/www/moodle/$dir" 2>/dev/null || true
            echo "  🔗 /var/www/moodle/$dir -> ${PUBLIC_DIR}/$dir"
        fi
    done
    
    # Lien pour config.php
    if [ -f "/var/www/moodle/config.php" ] && [ ! -e "${PUBLIC_DIR}/config.php" ]; then
        ln -sf /var/www/moodle/config.php "${PUBLIC_DIR}/config.php" 2>/dev/null || true
    fi
    
    # S'assurer que tout est bien owned
