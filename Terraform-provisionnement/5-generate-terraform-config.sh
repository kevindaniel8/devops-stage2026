...
ENV_FILE="${ENV_FILE:-/home/kevin/devops-stage2026/Terraform-provisionnement/env.conf}"
if [ -f "$ENV_FILE" ]; then
    source "$ENV_FILE"
...
PM_TOKEN_ID=$(ssh root@$PM_HOST "grep -o 'terraform[^!]*![^\"]*' /home/kevin-stage-devops/terraform-config.txt 2>/dev/null | head -1" 2>/dev/null || echo '') || true
...
PM_TOKEN_SECRET=$(ssh root@$PM_HOST "grep 'pm_token_secret' /home/kevin-stage-devops/terraform-config.txt 2>/dev/null | cut -d'=' -f2- | tr -d ' \"'" 2>/dev/null || echo '') || true
...
VM_CONFIG_FILE="${VM_CONFIG_FILE:-/home/kevin/devops-stage2026/Terraform-provisionnement/vm-definitions.json}"
